...
export default function AddUsers({visible,setVisible,tableData,setTableData}:IProps) {
    ...
    const searchBarItem: IFormItem[] = [
        ...
        },
        {
          ...
          render: (
            <>
            <Button className='bg-primary' type='primary' onClick={addUsers}>Add</Button>
            </>
          ),
        },
      ];
  ...
  return (
    <Modal width={1300} visible={visible} onCancel={()=>{setVisible(false)}} title='Select Member Account'>
        <h6>Select Member Accounts to upload to Group</h6>
        <SearchBar search={()=>{}} searchItem={searchBarItem}/>
        <Table rowSelection={rowSelection} columns={tableColumns} dataSource={userTableData} rowKey={'userId'}></Table>
    </Modal>
  )
}
