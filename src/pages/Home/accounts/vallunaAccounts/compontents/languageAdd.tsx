...
export default function languageAdd() {
  ...
  const { accountsInfoData,accountsInfoLanguageData,setAccountsInfoLanguageData, setAccountsInfoData } = useModel(
    'accountsInfo',
    (model: any) => ({
  ...
  return (
    <div className="text-center text-white">
      {accountsInfoLanguageData!==null &&accountsInfoLanguageData!==undefined && typeof(accountsInfoLanguageData) !== 'string'&&accountsInfoLanguageData.length !== 0 && accountsInfoLanguageData.map((item: ILanguage, index: number) => {
        ...
        return (
          <div key={item.language}>
            ...
            >
              {languageJson.map((item1, index) => {
                return (
                  <Select.Option
                    key={item1.code}
                    className={'bg-input-content rounded-lg text-white'}
                    value={item1.english}
                  >
                    {item1.english}
                  </Select.Option>
            ...
            <Select
              showSearch
              placeholder={(item.proficiency&&item.proficiency!==0)?getProficiency(item.proficiency):"Choose Proficiency"}
