...
import model from "@/models/accountsInfo"
export default function index(props: any) {
  const {setAccountsInfoData} = model()
  const accountsInfoData = getUserInfo()
    ...
    <div className="z-50 ">
      <div className="z-50 fixed bg-card-bg left-0 w-full h-[70px]  p-4 text-white  flex justify-between top-0">
        <div className="flex justify-start">
          ...
          }} src={headerLogo} width={303} height={45} alt="" />
        </div>
        <div>
          {accountsInfoData.username && (
            <Popover
              color="#8359da"
              ...
              content={
                <div className="w-28">
                  <div className="p-2 px-4 text-md  text-white text-center">
                    {accountsInfoData.username}
                  </div>
                  <div
                    onClick={pushInfo}
                    className="p-2 px-4 text-md text-center border-b border-gray-500 bg-black text-white cursor-pointer"
                  >
                    View Profile
                  </div>
                  <div
                    onClick={signOut}
                    className="rounded-b-lg p-2 px-4 text-md text-center bg-black text-white cursor-pointer"
                  >
                    Sign out
                  </div>
                </div>
